...
namespace Uniblocks
{
	...
	public class ChunkManager : MonoBehaviour
	{
		...
		public static bool CanSpawnChunk(VoxelInfo playerInfo, VoxelInfo chunkInfo)
		{
			var distance = Mathf.Round(Mathf.Sqrt(
										(playerInfo.index.x - chunkInfo.index.x) * (playerInfo.index.x - chunkInfo.index.x) +
										(playerInfo.index.y - chunkInfo.index.y) * (playerInfo.index.y - chunkInfo.index.y) +
										(playerInfo.index.z - chunkInfo.index.z) * (playerInfo.index.z - chunkInfo.index.z)
										));



			//if( distance <= Constants.Game.MAX_BLOCK_DISTANCE )
			//{
			//    return false;
			//}

			//return true;
			var isEqual = playerInfo.index.IsEqual(chunkInfo.index);

			var isChunkEqual = playerInfo.chunk == chunkInfo.chunk ? true : false;

			UnityEngine.Debug.Log("Distance: " + distance + " Equal: " + isEqual.ToString() + " Chunk Equal: " + isChunkEqual);
